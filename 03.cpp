...
int write(zggz a[],int k)   //保存职员信息
{
	...
	while(1)
	{
		...
		for(j=0;j<k;j++)
		{
			...
			{
				printf("已经有工号为%s的员工信息!请重新输入员工号。",gonghao);
				printf("请输入职工的工号：");
				scanf("%s",gonghao);
				strcpy(a[k].workerID,gonghao);
			}
...
void grsds(zggz a[],int n)				//计算个人所得税
{
	int i=0;
	if(n==0)
	{
	...
	do
	{
		...
		{
			a[i].personaltax=(a[i].sumsalary-100000)*0.45+29850;	//20000*1.3+15000*0.2+3000*0.15+1500*0.1+500*0.5
		}
	    else if(a[i].sumsalary>=80000.0)
		{
     		a[i].personaltax=(a[i].sumsalary-80000)*0.4+21850;
		}
    	else if(a[i].sumsalary>=60000)
		{
    		a[i].personaltax(a[i].sumsalary-60000)*0.35+14850;
		}
    	else if(a[i].sumsalary>=40000)
		{
		    a[i].personaltax=(a[i].sumsalary-40000)*0.3+8850;
		}
    	else if(a[i].sumsalary>=20000)
		{
	    	a[i].personaltax=(a[i].sumsalary-20000)*0.25+3850;
		}
    	else if(a[i].sumsalary>=5000)
		{
	    	a[i].personaltax=(a[i].sumsalary-5000)*0.2+850;
		}
	    else if(a[i].sumsalary>=2000)
		{
	    	a[i].personaltax=(a[i].sumsalary-2000)*0.15+400;
		}
	    else if(a[i].sumsalary>=500)
		{
	    	a[i].personaltax=(a[i].sumsalary-500)*0.1+25;
		}
	    else 
		{
	    	a[i].personaltax=a[i].sumsalary*0.05;
		}
	}while(i<n);
}
